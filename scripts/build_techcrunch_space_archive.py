"""TechCrunch Space 아카이브 빌더.

전략: /category/space/ RSS 피드 파싱 (feedparser). space datacenter 키워드 필터 적용.
      TechCrunch RSS가 최근 기사만 제공하므로, 아카이브가 점진적으로 누적됨.
      증분 업데이트: 기존 URL 재fetch 없음.

실행:
    python scripts/build_techcrunch_space_archive.py         # 최근 6개월
    python scripts/build_techcrunch_space_archive.py 12      # 최근 12개월

산출:
    data/archives/techcrunch_space.json
"""
import asyncio
import io
import json
...
ARCHIVE_PATH = ARCHIVE_DIR / "techcrunch_space.json"
SOURCE_NAME  = "TechCrunch"
SITE_BASE    = "https://techcrunch.com"
CATEGORY_BASE = SITE_BASE + "/category/space/page/{}/"
...
async def build(months: int) -> dict:
    ...
    async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True) as client:
        ...
        print(f"\n  [2/3] 기사 메타 fetch + 키워드 필터 (신규 {len(new_urls)}건, 동시 {CONCURRENCY})")
