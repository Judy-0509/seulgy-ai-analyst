...
전략: RSS 피드 파싱 (feedparser). space datacenter 키워드 필터 적용.
      증분 업데이트: 기존 URL 재fetch 없음.

실행:
    python scripts/build_spacecom_archive.py         # 최근 6개월
    python scripts/build_spacecom_archive.py 12      # 최근 12개월

산출:
    data/archives/spacecom.json
"""
import io
import json
import re
...
ARCHIVE_PATH = ARCHIVE_DIR / "spacecom.json"
SOURCE_NAME  = "Space.com"
SITE_BASE    = "https://www.space.com"
# Space.com Atom/RSS feed — returns a large batch of recent items
RSS_FEEDS = [
    "https://www.space.com/feeds/all",
    "https://www.space.com/rss/news.xml",
]
...
def build(months: int) -> dict:
    ...
    for rss_url in RSS_FEEDS:
        ...
        if not feed.entries:
            print(f"    {rss_url}: 기사 없음, 다음 URL 시도")
            time.sleep(CRAWL_DELAY)
            continue
        feed_ok = True
        print(f"    {rss_url}: {len(feed.entries)}건")
    ...
    print(f"  → 신규 관련 기사: {len(new_entries)}건")
