"""Data Center Knowledge 아카이브 빌더.

전략: RSS 피드 페이지네이션 (feedparser). space datacenter 키워드 필터 적용.
      증분 업데이트: 기존 URL 재fetch 없음.

실행:
    python scripts/build_datacenter_knowledge_archive.py         # 최근 6개월
    python scripts/build_datacenter_knowledge_archive.py 12      # 최근 12개월

산출:
    data/archives/datacenter_knowledge.json
"""
import io
import json
import re
...
ARCHIVE_PATH = ARCHIVE_DIR / "datacenter_knowledge.json"
SOURCE_NAME  = "Data Center Knowledge"
SITE_BASE    = "https://www.datacenterknowledge.com"
RSS_FEEDS = [
...
def _fetch_feed(base_url: str, cutoff: str, known_urls: set[str],
                keywords: list[str]) -> tuple[list[dict], bool]:
    """단일 RSS base URL에서 페이지네이션으로 수집. (entries, reached_cutoff) 반환."""
    entries: list[dict] = []
    stop_flag = False
...
def build(months: int) -> dict:
    ...
    for rss_url in RSS_FEEDS:
        test = feedparser.parse(rss_url)
        if not test.entries:
            print(f"    {rss_url}: 응답 없음, 다음 URL 시도")
            time.sleep(CRAWL_DELAY)
            continue
        feed_ok = True
        print(f"    사용 URL: {rss_url}")
        entries, _ = _fetch_feed(rss_url, cutoff, known_urls, keywords)
        new_entries.extend(entries)
        break

    if not feed_ok:
        print("  ⚠ 모든 RSS URL 응답 없음. 기존 archive 유지.")
        return {}
