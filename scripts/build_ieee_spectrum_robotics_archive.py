"""IEEE Spectrum Robotics 아카이브 빌더.

전략:
  - 번호형 sitemap 직접 순회 (sitemap_1 = 최신).
  - 미디어 파일(/media-library/) 제외, lastmod >= cutoff만 수집.
  - URL에 카테고리 경로 없으므로 기사 fetch 후 title/description 키워드 필터.
⚠ 본문: 일부 기사 IEEE 회원 전용. og:description 요약만 수집.

실행:
    python scripts/build_ieee_spectrum_robotics_archive.py         # 최근 6개월
    python scripts/build_ieee_spectrum_robotics_archive.py 12      # 최근 12개월

산출:
    data/archives/ieee_spectrum_robotics.json
"""
import asyncio
import io
import json
...
ARCHIVE_PATH = ARCHIVE_DIR / "ieee_spectrum_robotics.json"
SOURCE_NAME  = "IEEE Spectrum"
SITE_BASE    = "https://spectrum.ieee.org"
# sitemap_1 = 최신, 이후 번호는 오래된 내용이 산재
# sitemaps가 크로놀로지컬 순서가 아니므로 앞 N개만 시도
...
ROBOTICS_KEYWORDS = {
    "robot", "humanoid", "bipedal", "locomotion", "autonomous robot",
    "manipulation", "dexterous", "embodied", "boston dynamics", "figure ai",
    "optimus", "agility robotics", "unitree", "nvidia isaac",
    "legged", "warehouse robot", "industrial robot", "robotic arm",
    "robot hand", "walking robot",
}
...
def parse_sitemap(xml: str, cutoff: str) -> list[tuple[str, str]]:
    """미디어 제외, cutoff 이후 기사 URL + lastmod 반환."""
...
async def build(months: int) -> dict:
    ...
    async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True) as client:
        ...
        # 신규 URL fetch + 키워드 필터
        print(f"\n  [2/3] 기사 fetch + robotics 필터 ({len(new_pairs)}건, 동시 {CONCURRENCY})")
