...
전략:
  - Nikkei Asia 본문은 paywall (스크래핑 불가)
  - /rss/feed/nar 는 무료 (50건 제목 + URL, description은 비어있음)
  - 매일 실행해 누적 (증분)

사용:
  python scripts/build_nikkei_asia_archive.py
...
ARCHIVE_PATH = ARCHIVE_DIR / "nikkei_asia.json"
SITE_BASE = "https://asia.nikkei.com"
RSS_URL = SITE_BASE + "/rss/feed/nar"
...
def parse_rss(xml: str, fallback_lastmod: str) -> list[dict]:
    """Nikkei RSS (RDF 1.0)는 date 정보 부재 — fallback_lastmod 부여."""
    ...
    for item in soup.find_all("item"):
        ...
        date_el = item.find("date") or item.find("pubDate") or item.find("dc:date")
        if not title_el:
            continue
        ...
        if not url:
            url = item.get("rdf:about") or item.get("about") or ""
        if not url:
            continue
        ...
        date_text = date_el.text.strip() if date_el and date_el.text else ""
        if "T" in date_text:
            lastmod = date_text
        elif date_text:
            lastmod = parse_pubdate(date_text)
        else:
            lastmod = fallback_lastmod  # RSS 부재 시 빌드 시점 기록
...
async def build() -> dict:
    print("=" * 76)
    print("  Nikkei Asia Archive Builder")
    ...
    fallback_ts = datetime.now().isoformat(timespec="seconds")
    ...
    async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True, timeout=20) as c:
        ...
        if r.status_code != 200:
            ...
            return {}
        items = parse_rss(r.text, fallback_ts)
    ...
    archive = {
        ...
        "note": "RSS feed/nar — title only (description blocked by paywall); run daily to accumulate",
        "entries": merged,
    }
...
async def main():
    ...
    show_samples(
        archive,
        ["smartphone", "iPhone", "Samsung", "memory", "TSMC", "supply chain"],
    )
