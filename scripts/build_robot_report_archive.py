"""The Robot Report 아카이브 빌더.

전략: RSS 피드 수집 (공개 접근).
     사이트 자체는 Cloudflare로 페이지네이션/sitemap 완전 차단.
     RSS는 최신 15건만 제공 → 주기적 실행으로 점진 누적.

실행:
    python scripts/build_robot_report_archive.py

산출:
    data/archives/robot_report.json
"""
import io
import json
import re
...
ARCHIVE_PATH = ARCHIVE_DIR / "robot_report.json"
SOURCE_NAME  = "The Robot Report"
SITE_BASE    = "https://www.therobotreport.com"
...
def build() -> dict:
    ...
    archive = {
        ...
        "rss_url":          RSS_URL,
        "body_access":      "public",
        "note":             "Cloudflare 차단으로 RSS만 수집. 주기적 실행으로 점진 누적.",
