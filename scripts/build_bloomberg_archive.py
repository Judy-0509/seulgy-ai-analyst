"""Bloomberg Technology RSS 아카이브 빌더.

전략:
  - bloomberg.com 본문은 paywall + 봇 차단으로 직접 fetch 불가
  - feeds.bloomberg.com/technology/news.rss 는 무료, description = 1문단
  - 매번 ~30건 → 매일 실행해 누적 (증분)

사용:
  python scripts/build_bloomberg_archive.py
...
ARCHIVE_PATH = ARCHIVE_DIR / "bloomberg.json"
SITE_BASE = "https://www.bloomberg.com"
...
async def build() -> dict:
    print("=" * 76)
    print("  Bloomberg Technology Archive Builder")
    ...
    archive = {
        ...
        "note": "RSS technology/news.rss — run daily to accumulate (no historical archive)",
        "entries": merged,
    }
...
async def main():
    ...
    show_samples(
        archive,
        ["smartphone", "iPhone", "Apple", "Samsung", "AI", "memory"],
    )
