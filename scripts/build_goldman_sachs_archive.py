"""Goldman Sachs Research / Insights 아카이브 빌더 — 휴머노이드 / 로보틱스 / Physical AI.

전략: sitemap-1.xml 전체 수집 → URL slug에 humanoid/robot/automation/physical-ai 등
       관련 키워드가 있는 건만 필터 → og:title + og:description + lastmod 수집.

robots.txt 확인 (2026-05): User-agent:* 는 /materials/ 만 차단. /insights/ 는 허용.
GPTBot / ChatGPT-User 가 /insights/top-of-mind/ 등 일부 차단되나 일반 크롤러는 영향 없음.

실행:
    python scripts/build_goldman_sachs_archive.py

산출:
    data/archives/goldman_sachs.json
"""
import asyncio
import io
import json
...
ARCHIVE_PATH = ARCHIVE_DIR / "goldman_sachs.json"
SOURCE_NAME  = "Goldman Sachs Research"
SITE_BASE    = "https://www.goldmansachs.com"
SITEMAP_URLS = [
...
URL_KEYWORDS = [
    "humanoid", "robot", "physical-ai", "automation",
    "ai-accelerant", "embodied",
]
...
async def build_async() -> dict:
    ...
    out = {
        ...
        "body_note":         "og:title / og:description 만 저장 (보고서 본문은 paywall)",
