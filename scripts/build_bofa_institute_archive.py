"""Bank of America Institute 아카이브 빌더 — 휴머노이드 / Physical AI / Transformation.

전략: sitemap 1개에 156개 URL 정도 → /transformation/ 경로의 humanoid/robot/physical-ai
       관련 페이지만 필터 → og:title + og:description + lastmod 수집.

robots.txt 확인 (2026-05): /cgi-bin/, /tmp/ 만 차단. 나머지 전체 허용.
Sitemap: institute.bankofamerica.com/content/institute/bank-of-america-institute.sitemap.xml

실행:
    python scripts/build_bofa_institute_archive.py

산출:
    data/archives/bofa_institute.json
"""
import asyncio
import io
import json
...
ARCHIVE_PATH = ARCHIVE_DIR / "bofa_institute.json"
SOURCE_NAME  = "Bank of America Institute"
SITE_BASE    = "https://institute.bankofamerica.com"
SITEMAP_URL  = ("https://institute.bankofamerica.com/content/institute/"
...
async def build_async() -> dict:
    ...
    out = {
        ...
        "body_access":       "public",
        "body_note":         "전체 페이지 무료 공개. og:meta 우선 수집.",
