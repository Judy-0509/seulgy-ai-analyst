"""Data Center Frontier 아카이브 빌더.

전략: RSS 피드 페이지네이션 (feedparser). space datacenter 키워드 필터 적용.
      증분 업데이트: 기존 URL 재fetch 없음.

실행:
    python scripts/build_datacenter_frontier_archive.py         # 최근 6개월
    python scripts/build_datacenter_frontier_archive.py 12      # 최근 12개월

산출:
    data/archives/datacenter_frontier.json
"""
import io
import json
import re
...
ARCHIVE_PATH = ARCHIVE_DIR / "datacenter_frontier.json"
SOURCE_NAME  = "Data Center Frontier"
SITE_BASE    = "https://www.datacenterfrontier.com"
RSS_FEEDS = [
    # Custom CMS RSS endpoint (Informa/EB Media)
...
def build(months: int) -> dict:
    ...
    for page in range(1, MAX_PAGES + 1):
        if stop_flag:
            break
        url = active_url if page == 1 else f"{active_url}?paged={page}"
