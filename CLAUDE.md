...
스마트폰·휴머노이드·자동차·스마트글래스 시장 리서치 자동화 도구.
...
| 스마트글래스 메이저 (30일) | `scripts/_smartglass_topic_suggestions.json` | `GET /api/topics/suggested?domain=smartglass` |
| 스마트글래스 Curiosity (7일) | `scripts/_smartglass_topic_suggestions_emerging.json` | `↑ 같은 응답에 합쳐짐` |
...
# 개별 소스 — 스마트글래스 (8개 신규 + 재활용 10개)
python scripts/build_uploadvr_archive.py
python scripts/build_skarredghost_archive.py
python scripts/build_roadtovr_archive.py
python scripts/build_arinsider_archive.py        # robots Crawl-delay 10s — 느림 (정상)
python scripts/build_kgontech_archive.py         # 월 1건 저빈도 — 주 1회면 충분
python scripts/build_meta_newsroom_archive.py    # 전사 뉴스룸 — 강한 키워드 필터
python scripts/build_rokid_archive.py            # Shopify sitemap, lastmod 날짜
python scripts/build_citi_archive.py             # IB — __NEXT_DATA__ publishDate
# + 재활용: 트래커 7(Counterpoint·Omdia·TrendForce·IDC·CCS·Yole·DigiTimes)
#   + ABI/IDTechEx/BofA(humanoid 빌더, 필터 union 확장) — 별도 빌드 불필요
```
...
│   │
│   │   # 스마트글래스 아카이브 (8개 신규 빌더)
│   ├── _smartglass_research_helper.py  ← 스마트글래스 빌더 공통 헬퍼 (word-boundary 매처)
│   ├── build_uploadvr_archive.py       ← UploadVR (XR 전문 매체)
│   ├── build_skarredghost_archive.py   ← The Ghost Howls (XR 전문 매체)
│   ├── build_roadtovr_archive.py       ← Road to VR (XR 전문 매체)
│   ├── build_arinsider_archive.py      ← AR Insider (XR 애널리스트 매체)
│   ├── build_kgontech_archive.py       ← KGOnTech (광학·디스플레이 심층 분석)
│   ├── build_meta_newsroom_archive.py  ← Meta Newsroom (about.fb.com, 키워드 필터)
│   ├── build_rokid_archive.py          ← Rokid (벤더 공식, Shopify sitemap)
│   ├── build_citi_archive.py           ← Citi Research (IB, __NEXT_DATA__ 날짜)
│   │   # 스마트글래스 주제 추천
│   ├── suggest_smartglass_topics.py    ← 스마트글래스 메이저 → _smartglass_topic_suggestions.json
│   ├── suggest_smartglass_emerging.py  ← 스마트글래스 Curiosity → _smartglass_topic_suggestions_emerging.json
│   │
...
│   ├── smartglass_keywords.json    ← 스마트글래스 필터링 키워드 (46개)
